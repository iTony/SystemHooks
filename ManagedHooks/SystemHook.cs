...
namespace Kennedy.ManagedHooks
{
	...
	public abstract class SystemHook : IDisposable
	{
		...
		{
			if (!InternalGetMousePosition(wparam, lparam, ref x, ref y,ref hwnd))
			{
		...
		[DllImport("SystemHookCore.dll", EntryPoint="GetMousePosition",  SetLastError=true,
			 CharSet=CharSet.Unicode, ExactSpelling=true,
			 CallingConvention=CallingConvention.StdCall)]
		private static extern bool InternalGetMousePosition(UIntPtr wparam, IntPtr lparam, ref int x, ref int y,ref IntPtr hwnd);
